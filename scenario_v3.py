"""
Szenario v3 - DICHTE Variante. Eigene Konfiguration, eigene Auswertung.

Entstanden aus zwei gemessenen Kontrollbedingungen:
  v1  Solo-A 3/10 korrekt - Set A enthielt mit "degraded mode ... 40-60s" die
      halbe Ursache frei Haus.
  v2  Solo-A 0/10, aber Solo-B 2/10 - der Change-Kalender bei B trug die
      Latenz UND der Diff die Pool-Grenze, damit war B's Set fuer sich
      geschlossen.
  v3  Der Change-Kalender ist entfernt, die Blockadedauer liegt als
      Call-Dauer bei A, die Pool-Grenze nur noch im Log bei A, der Diff
      enthaelt zusaetzlich zwei harmlose Hunks als Rauschen. Jede Seite haelt
      genau ein unverzichtbares Stueck.
...
[billing-api] 10:07:55  INFO   outbound POST tax-service /v1/quote  ... still open (28.9s)
[billing-api] 10:08:20  INFO   outbound POST tax-service /v1/quote  ... still open (41.2s)
[billing-api] 10:08:44  WARN   outbound POST tax-service /v1/quote  completed 57.3s status=200
[billing-api] 10:12:07  WARN   outbound POST tax-service /v1/quote  completed 59.8s status=200

[billing-api] 10:41:59  INFO   db pool: recovered (in_use=3 max=20 queued=0)
--- ENDE LOGAUSSCHNITTE ---

Logs anderer Dienste, Konfigurationswerte und die Aenderungshistorie stehen
...
+      request_timeout: 60s     # NW-4471: tax-service laeuft gelegentlich in 5s-Timeout
       retries: 0
     shipping_service:
       endpoint: https://shipping.internal/v2
-      request_timeout: 8s
+      request_timeout: 10s     # NW-4468
       retries: 2

   logging:
-    level: info
+    level: warn                # NW-4455: Logvolumen senken
...
tax-service von 5s auf 60s erhoeht (nur im Diff, also nur bei B sichtbar).
tax-service brauchte ab 10:00 tatsaechlich bis zu 60s pro Antwort (nur im
Log, also nur bei A sichtbar). Jeder Checkout haelt waehrend des
tax-service-Calls eine Verbindung aus dem DB-Pool mit max=20 (nur im Log,
also nur bei A sichtbar). Statt nach 5s freizugeben, blockieren die Calls nun
bis zu 60s. Der Pool laeuft voll, das Gateway schlaegt nach 30s mit 504 fehl.

Dicht geteilt - jede Seite haelt genau ein unverzichtbares Stueck:
- Nur A weiss, dass ueberhaupt ein Ausfall stattfand, dass der Pool (max=20)
  volllief und dass die tax-service-Calls bis zu 59,8s offen blieben.
- Nur B weiss, dass der Timeout gegenueber tax-service von 5s auf 60s
  heraufgesetzt wurde - und dass v2.14.0 ueberhaupt Timeouts angefasst hat.
- A allein bleibt zwingend bei der Falle: die langsame Query ist die einzige
  Erklaerung, die sein Set hergibt. Er kann nicht wissen, dass 59,8s vorher
  nach 5s abgebrochen worden waeren.
- B allein sieht nur einen erhoehten Timeout ohne jeden Hinweis auf einen
  Vorfall, ohne Pool-Grenze und ohne Latenzen - drei Diff-Hunks, von denen
  keiner fuer sich einen Ausfall belegt.
"""
