...
SCENARIO_ID = "billing-504-pool-timeout"
SCENARIO_VERSION = 1
...
[tax-service] 10:00:00  INFO   degraded mode: upstream vat-registry slow, responses 40-60s
[tax-service] 10:41:58  INFO   degraded mode cleared
...
tax-service von 5s auf 60s erhoeht (nur im Diff sichtbar). tax-service lief ab
10:00 im degraded mode mit Antwortzeiten von 40-60s (nur im Log sichtbar).
Jeder Checkout haelt waehrend des tax-service-Calls eine Verbindung aus dem
DB-Pool (max=20). Statt nach 5s freizugeben, blockieren die Calls nun bis zu
60s. Der Pool laeuft voll, Requests stauen sich, das Gateway schlaegt nach 30s
mit 504 fehl. Erst als tax-service um 10:41 den degraded mode verlaesst,
erholt sich der Pool.

Keine Seite kann das allein feststellen:
- A sieht die Pool-Erschoepfung und die langsamen tax-service-Calls, kennt aber
  den Timeout-Wert und dessen Aenderung nicht.
- B sieht die Timeout-Aenderung, weiss aber nicht, dass es einen Ausfall gab,
  dass tax-service degradiert war oder dass der Pool voll lief.
"""
...
SOLUTION_REQUIRED_GROUPS = [
    # 1) die Timeout-Erhoehung
    ["60s", "60 s", "60 sekunden", "5s auf 60", "5 auf 60", "request_timeout",
     "timeout-erhoehung", "timeout erhöht", "timeout erhoeht", "erhöhten timeout",
     "erhöhte timeout", "client-timeout", "http-client-timeout"],
